...
## 7 · EL SÍMBOLO: V4, y por qué (integrado)

El color (§6) es solo la mitad. El símbolo es una **Z** —de ZetaBus y de Zaragoza— trazada como un recorrido, con la base prolongada y un **poste de parada** plantado encima: *el recorrido llega a la parada*. Hereda de Linaje el vocabulario de construcción (líneas de punta redonda + un nodo), y es propio el color y el dibujo.

### V4 elegida

Cinco variantes dibujadas y medidas (ronda 2). Gana **V4** —señal en bandera, prolongación media— por un motivo concreto: **gana en las pantallas donde el logo se USA** (la pestaña del navegador, el icono de la app, un avatar, la cabecera compacta), no solo donde se mira ampliado. Es la más unida a tamaño pequeño (se lee como un glifo, no como «dos cosas»), la más alta en un cuadrado, y su Z sirve de favicon sin retoques.

### V5 descartada — con sus números, que eran menores

V5 (prolongación larga + bandera) contaba la historia entera, y se descartó **pagando un precio conocido, no por ser peor**:

- **Z un 13% más baja** en caja cuadrada (180 px vs 206 px de V4, mismo margen): la prolongación larga obliga a encoger la marca para que quepa el ancho.
- **Hueco cuerpo-Z ↔ poste del 14,6%** del ancho, contra 6,8% de V4: a tamaño pequeño se lee un pelín más como dos elementos (la bandera lo mitiga, no lo borra).
- **Favicon ladeado**: sin poste, su base larga queda caída a un lado.

Ninguno es grave; sumados, inclinaron la balanza hacia donde el logo trabaja de verdad.

### El bitono en gris — dicho sin adornar

Los dos tonos (`#7048E8` Z / `#4E22B8` poste) desaturados quedan a **1,68:1 entre sí**. ⚠️ Ese suelo de **1,5:1 fue un número puesto a ojo, no una norma** — no hay un WCAG del "bitono". Lo que de verdad sostiene la marca en monocromo **no es el contraste entre los dos tonos**, sino que **cada tono pega fuerte contra el fondo**: la Z a **5,55:1 sobre papel y 4,51:1 sobre el lienzo**, el poste a 9,35:1 y 7,59:1 (medido sobre el píxel resultante, sin `opacity` que lo infle). El bitono es un **matiz de profundidad suave que sobrevive** al gris, no un canal de información. En gris el logo sigue siendo un logo legible; el segundo tono es un lujo, no una muleta.

### Los contratrazos, y por qué el favicon es un recorte de la marca

La Z sola a 16 px conserva sus huecos interiores abiertos al **42–49% del bounding box, con los dos grosores** (6 y 8). Ninguna se cierra en un cuadrado. Eso es lo que permite la **Opción A**: una sola geometría (`Z_PATH` en `src/components/marca-fuente.ts`), el stroke como única variable (6 en la marca, 8 en el favicon). El favicon **no es un SVG aparte**: `app/icon.tsx` interpola el mismo path. Que no vuelvan a existir dos Z lo vigila `tests/marca-z-unica.test.ts` (caza cualquier Z nueva por su forma, no por una lista de sitios). La Z bespoke anterior (15/43/49) se retiró.
